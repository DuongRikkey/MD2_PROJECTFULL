...
//                            List<OrderDetails> orderDetailsList=OrderDetailsImpl.orderDetailsList.stream().filter(orderDetails ->orderDetails.getOrderId()==newIdSelect).toList();
//                            for (OrderDetails orderDetail : orderDetailsList) {
//                                Optional<Product> optionalProduct=ProductImpl.productList.stream().filter(product -> product.getProductId()==orderDetail.getProductId()).findFirst();
//                                if (optionalProduct.isPresent()) {
//                                    Product product=optionalProduct.get();
...
//                                }
//                            }
//                            int totalPriceOfOrder= OrderDetailsImpl.orderDetailsList.stream().
//                                    mapToInt(orderDetails ->
//                                    {Optional<Product> optionalProduct=ProductImpl.productList.
//                                            stream().filter(product -> product.getProductId()==orderDetails.getProductId()).findFirst();
//                                        if (optionalProduct.isPresent()) {
//                                            Product product=optionalProduct.get();
//                                            return (int) (product.getUnitPrice()*orderDetails.getOrder_quantity());
//                                        }
//                                        return 0;
//                                    }
//
//                            ).sum();
//                            Main.userLogin.setWallet(Main.userLogin.getWallet()+totalPriceOfOrder);
//                            IOFILE.writeListToFile(UsersImpl.usersList,ShopConstanst.USER_PATH);
                        ...
                        if (isValidTransition(currentStatus, Status.DENIED)) {
                            ...
                            // Get order details for the specific order
                            ...
                            // Restore product stock based on order details
                            for (OrderDetails orderDetail : orderDetailsList) {
                                Optional<Product> optionalProduct = ProductImpl.productList.stream()
                            ...
                            Optional<Users> optionalUser = UsersImpl.usersList.stream()
                                    .filter(user -> user.getId() == order.getUserId()) // Assuming order contains the userId
                                    .findFirst();
                            ...
                            if (optionalUser.isPresent()) {
                                ...
                                // Refund the user by adding the total order price back to their wallet
                                customer.setWallet(customer.getWallet() + totalPriceOfOrder);
                                System.out.println("Số tiền " + totalPriceOfOrder + " đã được hoàn lại vào ví của khách hàng " + customer.getUsername());

                                // Save updated user and product data to the file
                                IOFILE.writeListToFile(UsersImpl.usersList, ShopConstanst.USER_PATH);
                                IOFILE.writeListToFile(ProductImpl.productList, ShopConstanst.PRODUCT_PATH);
                            } else {
